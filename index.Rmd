...
data_batch2_mv_data0 <- data_batch2[,str_detect(colnames(data_batch2),"_score")]
```
...
```{r Conduct MICE for SBQ-LC in batch 1 & 2 based on batch 2, include=FALSE}
data_batch1_2_preval_data1 <-
  data_batch1_2_preval_data0 |>
  mice(
    m = 10
    , method = "norm.predict"
...
data_batch2_moca_ina_mv_data1 <-
  suppressWarnings(
    cbind(
      select(data_batch2_moca_ina_mv_data1, id,age_timestamp,age_inf_date)
      , data_batch2_moca_ina_mv_data1 |>
        select(-id, -age_timestamp, -age_inf_date) |>
        assign_missing(var_med_iqr = trans_data$none_med_iqr) |>
        mice(m = 10, method = "pmm", seed = seed, print = FALSE) |>
...
data_batch2_moca_ina_desc2_ver_outcome <-
  data_batch2_moca_ina_mv_data1 |>
  desc_stats_vertical(p_row = TRUE) |>
  mutate(variable=ifelse(variable == "batch", "moca_ina_class", variable))
```
...
data_batch2_moca_ina_desc3_ver <-
  data_batch2_moca_ina_desc2_ver |>
  left_join(variable_label_type, by = join_by(variable)) |>
  mutate(variable = label) |>
  select(-label) |>
...
data_batch2_moca_ina_mv_data2 <-
  ...
  arrange(interview_onset) |>
  gather(batch, outcome, -id, -interview_onset) |>
  filter(batch != levels(data_batch2_moca_ina_mv_data1$batch)[1]) |>
  right_join(
    select(data_batch2_moca_ina_mv_data1, -interview_onset, -batch)
    , by = join_by(id)
  )
```
...
data_batch2_moca_ina_mv_data <- data_batch2_moca_ina_mv_data2
```
...
```{r figure-s10, echo=FALSE, fig.height=7, fig.width=11}
figures10
```
...
figures10
ggsave("doc/figure10.eps", height = 7, width = 11)
```
...
```{r figure-s11, echo=FALSE, fig.height=7, fig.width=11}
figures11
```
...
figures11
ggsave("doc/figures11.eps", height = 7, width = 11)
```
...
interlr_results <-
  list(
    ...
    , `CI positive`=
        ...
        intervar_lr(data_batch2_moca_ina_mv_data)
  )
```
...
multicox_variables$`Cluster 2+3 ~ exposure`$covariates <-
  multicox_variables$`Cluster 2+3 ~ exposure`$covariates[
    !multicox_variables$`Cluster 2+3 ~ exposure`$covariates
     %in% c("vaccine", "other_sore_throat", "other_fever", "age_timestamp")
  ]
...
multicox_variables$`CI positive ~ exposure`$data <- data_batch2_moca_ina_mv_data
...
multicox_variables$`CI positive ~ exposure`$covariates <-
  multicox_variables$`CI positive ~ exposure`$covariates[
    !multicox_variables$`CI positive ~ exposure`$covariates
     %in% c("vaccine", "other_sore_throat", "age_timestamp")
  ]

# CI positive ~ vaccine
multicox_variables$`CI positive ~ vaccine`$data <- data_batch2_moca_ina_mv_data
  ...
  imap(
    ~ multicox_variables[[.y]]$data |>
      multivar_coxph(
        ...
        , unicox_result_df =
          unicox_results[[
            str_remove_all(.y, " ~ exposure| ~ vaccine| \\+ Cluster 2\\+3")
          ]]
      )
  )
```
...
```{r Exposure as causal factor of cluster 2+3, include=FALSE}
tables19 <-
  multicox_results_sum[1:4] |>
  imap(~ mutate(.x, effect_cause = .y)) |>
  c(multicox_results_sum[5] |>
  ...
  mutate(
    category =
      case_when(
        ...
        ~ paste0(
            "Moderate-to-severe COVID-19 at any time or mild COVID-19 within "
            , "the past year"
          )
...
data_batch1_2_preval_data <-
  data_batch1_2_preval_data1 |>
  mutate(seq = seq(nrow(data_batch1_2_preval_data1))) |>
  gather(colname, new_score, -seq)

data_batch1_2_preval_data <-
  data_batch1_2_preval_data |>
  rbind(
    group_by(data_batch1_2_preval_data, seq) |>
      summarize(new_score = mean(is.na(new_score)) == 0) |>
      mutate(colname = "complete") |>
      select(colname, new_score, everything())
  ) |>
...
data_batch1_2_preval_data <-
  data_batch1_2_preval_data %*% data_batch1_pca$rotation[, "PC1", drop = FALSE]

data_batch1_2_preval_data <-
  as.data.frame(data_batch1_2_preval_data) |>
  rownames_to_column(var = "seq") |>
  mutate(seq = as.numeric(seq)) |>
  rename(sbq_lc_total = PC1) |>
  right_join(
  ...
  mutate(
    sbq_lc_class =
      case_when(
        sbq_lc_total > pc1_cl_intersections$`Cluster 1-2` ~ "Cluster 1"
        ,sbq_lc_total <= pc1_cl_intersections$`Cluster 1-2` ~ "Cluster 2+3"
      ) |>
      factor(c("Cluster 1", "Cluster 2+3"))
  ) |>
  left_join(
    data_batch1_2 |>
      mutate(
      ...
      select(id, moca_ina_total, moca_ina_class) |>
      mutate(seq = seq(nrow(data_batch1_2)))
    , by = join_by(seq)
  ) |>
  arrange(seq) |>
...
for(i in seq(1000)){
  ...
  preval_boot[[i]] <-
    preval_boot[[i]] |>
    rbind(
      mutate(preval_boot[[i]], sbq_lc_class = "Cluster 1+2+3") |>
  ...
  preval_boot[[i]] <-
    preval_boot[[i]] |>
    rbind(
      mutate(preval_boot[[i]], moca_ina_class = "CI total") |>
        group_by(sbq_lc_class, moca_ina_class, total) |>
        summarize(n = sum(n), .groups = "drop")
    )
}
...
tables20 <-
  tables20 |>
  rbind(
    mutate(tables20, sbq_lc_class = "Cluster 1+2+3") |>
      group_by(sbq_lc_class, moca_ina_class, total) |>
      summarize(n = sum(n), .groups = "drop")
  )
...
tables20 <-
  tables20 |>
  rbind(
    mutate(tables20, moca_ina_class = "CI total") |>
      group_by(sbq_lc_class, moca_ina_class, total) |>
      summarize(n = sum(n), .groups = "drop")
  ) |>
  mutate(p = n / total) |>
...
for(i in seq(1000)){
  ...
  preval_boot_batch2[[i]] <-
    preval_boot_batch2[[i]] |>
    rbind(
      mutate(preval_boot_batch2[[i]], sbq_lc_class = "Cluster 1+2+3") |>
        group_by(sbq_lc_class, moca_ina_class, total) |>
        summarize(n = sum(n), .groups = "drop")
    )
  ...
  preval_boot_batch2[[i]] <-
    preval_boot_batch2[[i]] |>
    rbind(
      mutate(preval_boot_batch2[[i]], moca_ina_class = "CI total") |>
        group_by(sbq_lc_class, moca_ina_class, total) |>
        summarize(n = sum(n), .groups = "drop")
    )
}
...
tables21 <-
  tables21 |>
  rbind(
    mutate(tables21, sbq_lc_class = "Cluster 1+2+3") |>
      group_by(sbq_lc_class, moca_ina_class, total) |>
      summarize(n = sum(n), .groups = "drop")
  )
...
tables21 <-
  tables21 |>
  rbind(
    mutate(tables21, moca_ina_class = "CI total") |>
      group_by(sbq_lc_class, moca_ina_class, total) |>
      summarize(n = sum(n), .groups = "drop")
  ) |>
  mutate(p = n / total) |>
...
```{r Bootstrap to estimate prevalence in batch 2 x infection, include=FALSE}
preval_boot_batch2_infection <- list()

for(i in seq(1000)){
  set.seed(seed + i)
  
  preval_boot_batch2_infection[[i]] <-
    data_batch1_2_preval_data_plus |>
    slice(
      sample(
  ...
  preval_boot_batch2_infection[[i]] <-
    preval_boot_batch2_infection[[i]] |>
    rbind(
      mutate(preval_boot_batch2_infection[[i]], exposure = "All") |>
  ...
  preval_boot_batch2_infection[[i]] <-
    preval_boot_batch2_infection[[i]] |>
    rbind(
      mutate(preval_boot_batch2_infection[[i]], moca_ina_class = "CI total") |>
        group_by(exposure, moca_ina_class, total) |>
        summarize(n = sum(n), .groups = "drop")
    )
}
...
```{r Prevalence of infection and CI in batch 2, include=FALSE}
tables22 <-
  data_batch1_2_preval_data_plus |>
  group_by(exposure, moca_ina_class) |>
  summarize(n = n(), .groups = "drop") |>
...
tables22 <-
  tables22 |>
  rbind(
    mutate(tables22, moca_ina_class = "CI total") |>
      group_by(exposure, moca_ina_class, total) |>
      summarize(n = sum(n), .groups = "drop")
  ) |>
  mutate(p = n / total) |>
  left_join(
    preval_boot_batch2_infection |>
      reduce(rbind) |>
      mutate(p = n / total) |>
      group_by(exposure, moca_ina_class) |>
      summarize(
...
tables22 |>
  kable(
    format = "html"
    , caption = "Table S22. Prevalence of infection and CI in batch 2"
  ) |>
  kable_classic() |>
  kable_styling(position="float_left")
```
...
```{r Bootstrap to estimate prevalence in batch 2 x vaccine, include=FALSE}
preval_boot_batch2_vaccine <- list()

for(i in seq(1000)){
  set.seed(seed + i)
  
  preval_boot_batch2_vaccine[[i]] <-
    data_batch1_2_preval_data_plus |>
    slice(
      sample(
        seq(nrow(data_batch1_2_preval_data_plus))
  ...
  preval_boot_batch2_vaccine[[i]] <-
    preval_boot_batch2_vaccine[[i]] |>
    rbind(
      mutate(preval_boot_batch2_vaccine[[i]], vaccine = "All") |>
  ...
  preval_boot_batch2_vaccine[[i]] <-
    preval_boot_batch2_vaccine[[i]] |>
    rbind(
      mutate(preval_boot_batch2_vaccine[[i]], moca_ina_class = "CI total") |>
        group_by(vaccine, moca_ina_class, total) |>
        summarize(n = sum(n), .groups = "drop")
    )
}
...
```{r Prevalence of vaccine and CI in batch 2, include=FALSE}
tables23 <-
  data_batch1_2_preval_data_plus |>
  group_by(vaccine, moca_ina_class) |>
  summarize(n = n(), .groups = "drop") |>
...
tables23 <-
  tables23 |>
  rbind(
    mutate(tables23, moca_ina_class = "CI total") |>
      group_by(vaccine, moca_ina_class, total) |>
      summarize(n = sum(n), .groups = "drop")
  ) |>
  mutate(p = n / total) |>
  left_join(
    preval_boot_batch2_vaccine |>
      reduce(rbind) |>
      mutate(p = n / total) |>
      group_by(vaccine, moca_ina_class) |>
      summarize(
...
tables23 |>
  kable(
    format = "html"
    , caption = "Table S23. Prevalence of vaccine and CI in batch 2"
  ) |>
  kable_classic() |>
  kable_styling(position="float_left")
```
...
```{r figure-s15, echo=FALSE, fig.height=7, fig.width=11}
figures15
```
...
figures15
ggsave("doc/figures15.eps", height = 7, width = 11)
```
